...
@section('title', 'Tambah Kriteria - Sistem Prioritas Perbaikan Jalan')
@section('page-title', 'Tambah Kriteria Baru')
@section('page-subtitle', 'Isi formulir di bawah ini untuk menambahkan kriteria penilaian baru')

@section('content')
<div class="form-container">
    <form method="POST" action="{{ route('admin.kriteria.store') }}" id="formKriteria">
        @csrf
            ...
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    ...
                    <p style="margin: 5px 0 0; color: var(--text-light); font-size: 13px;">
                        ...
                        Saat ini total bobot kriteria aktif: <strong id="currentTotalBobot">0%</strong>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 24px;">
            ...
            <div>
                <div class="form-card">
                    ...
                    <!-- Kode Kriteria (Otomatis) -->
                    <div class="form-group">
                        <label class="form-label">
                            Kode Kriteria <span class="text-danger">*</span>
                            <span class="label-info">(Otomatis)</span>
                        </label>
                        ...
                        <div class="kode-status" style="margin-top: 5px; font-size: 12px; color: #10B981;">
                            <i class="fas fa-magic"></i> Kode otomatis dihasilkan sistem
                        </div>
                        @error('kode') 
                    ...
                    <div class="form-group">
                        ...
                        <div class="radio-group">
                            <label class="radio-option">
                                <input type="radio" name="tipe" value="benefit" {{ old('tipe') == 'benefit' ? 'checked' : '' }}>
                            ...
                            <label class="radio-option">
                                <input type="radio" name="tipe" value="cost" {{ old('tipe') == 'cost' ? 'checked' : '' }}>
                ...
                <div class="form-card">
                    ...
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                            ...
                            <div style="width: 120px; text-align: center;">
                                <div id="bobotPreview" style="font-size: 24px; font-weight: 700; color: var(--secondary);">0%</div>
                            </div>
                        </div>
                        <div id="bobotSlider" style="margin-top: 10px;"></div>
                    ...
                    <div style="background: #F1F5F9; border-radius: 10px; padding: 12px; margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 13px;">
                            <span>Total Bobot Aktif Saat Ini:</span>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" 
                                  rows="5" 
                                  class="form-control"
                                  placeholder="Jelaskan kriteria ini, cara pengukuran, skala penilaian, dll...">{{ old('keterangan') }}</textarea>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let totalBobotAktif = {{ $totalBobotAktif ?? 0 }};
    let currentBobot = 0;
    
    // Update total bobot display
    function updateTotalBobot() {
        let newTotal = totalBobotAktif + currentBobot;
        ...
        if (newTotal > 1) {
            ...
            totalBobotMessage.innerHTML = '<i class="fas fa-check-circle"></i> Total bobot akan tepat 100% setelah ditambahkan';
            totalBobotMessage.style.color = '#10B981';
            bobotWarning.innerHTML = '';
        } else {
            totalBobotBar.style.background = 'var(--secondary)';
            totalBobotMessage.innerHTML = '<i class="fas fa-info-circle"></i> Total bobot akan ' + percent + '% setelah ditambahkan';
    ...
    if (bobotInput) {
        function parseBobot(value) {
            // If value contains % or is > 1, treat as percentage
    ...
    if (urutanInput) {
        urutanInput.addEventListener('input', function() {
            ...
            urutanTimeout = setTimeout(function() {
                fetch(`/admin/kriteria/cek-urutan?urutan=${urutan}`)
    ...
    // Initialize total bobot display from PHP
    function initTotalBobot() {
        let initialTotal = {{ $totalBobotAktif ?? 0 }};
        let initialPercent = (initialTotal * 100).toFixed(0);
        totalBobotDisplay.innerHTML = initialPercent + '%';
        totalBobotBar.style.width = Math.min(initialPercent, 100) + '%';
        
        if (initialTotal === 1) {
            totalBobotBar.style.background = '#10B981';
            totalBobotMessage.innerHTML = '<i class="fas fa-check-circle"></i> Bobot sudah 100% sempurna!';
            totalBobotMessage.style.color = '#10B981';
        } else if (initialTotal > 1) {
            totalBobotBar.style.background = '#EF4444';
            totalBobotMessage.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Bobot melebihi 100%! Silakan nonaktifkan beberapa kriteria.';
            totalBobotMessage.style.color = '#EF4444';
        } else {
            totalBobotBar.style.background = 'var(--secondary)';
            let remaining = ((1 - initialTotal) * 100).toFixed(0);
